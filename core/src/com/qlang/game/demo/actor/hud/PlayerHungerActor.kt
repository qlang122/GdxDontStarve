...
class PlayerHungerActor(val manager: AssetManager) : Actor() {
    ...
    init {
        val file = Gdx.files.internal(R.anim.hud.hunger)
        val parameters = SCMLLoader.Parameters(R.anim.hud.hunger_atlas)
        val scml = SCMLLoader(InternalFileHandleResolver()).load(manager, "", file, parameters)
        val entity = scml.getEntity("hunger")
...
//            p = p.parent
//        }
////        Log.e("QL", "--2-->>", x, y, this.x, this.y, offsetX, offsetY, width, height)
